...
Тема истории: {topic}  # Добавляем тему для связного сюжета

Напиши короткую историю уровня A1-A2 на 5-7 предложений.
Главный приоритет — логичная, естественная история, похожая на реальные бытовые ситуации.

Используй как можно больше слов из списка: {', '.join(words)}
Если какое-либо слово делает историю неестественной, его можно пропустить.

Требования:

- История должна иметь сюжет: начало, развитие, завершение.
- Все события логически связаны.
- Можно изменять форму слов по правилам турецкого языка.
- Используй естественные формы слов вместо инфинитивов.
- Каждое слово из списка должно быть использовано хотя бы один раз, если это возможно.
- Выделяй использованные слова тегами <b></b>, не используй Markdown (**).
- Не делай грамматический разбор.
- Не объясняй правила.
- Не добавляй комментарии, вступления или заключения.

Примеры изменения форм слов:
